...
**System Version**: v2.1 - SSE + HTTP Transport Migration Complete  

### **🚀 Recent Migration: WebSocket → HTTP Transport for Agent Communication**

**Key Architecture Changes:**
- ✅ **Agent-to-Agent Communication**: Migrated from WebSocket to **HTTP + JSON-RPC** for MCP protocol
- ✅ **UI Real-time Updates**: WebSocket remains for user interface live updates (unchanged)  
- ✅ **MCP Endpoints**: All agents now expose HTTP-based MCP endpoints (`/mcp`)
- ✅ **Protocol Distinction**: Clear separation between HTTP (agent communication) and WebSocket (UI updates)

**Impact**: Improved reliability, better error handling, and simplified deployment architecture for inter-agent communication.
...
- **HTTP Transport**: MCP over HTTP for reliable agent-to-agent communication
...
flowchart TD
    ...
    ActionMgr -->|No| DirectCall[📞 Direct HTTP/MCP Call]
    ...
    DirectCall -->|HTTP/MCP| MCPInfra[🔧 Infrastructure<br/>Agent + K8s AI]
    DirectCall -->|HTTP/MCP| MCPObs[📊 Observability<br/>Agent + SLM]
...
| Workers ↔ Agents | MCP over HTTP | Tool execution | ✅ Active |
...
- ✅ **MCP over HTTP Orchestration**: Coordinates multiple agent interactions
...
sequenceDiagram
    ...
    IW->>IA: Execute deployApplication via MCP over HTTP
...
sequenceDiagram
    ...
    M->>IA: Direct MCP over HTTP (getResourceStatus)
...
| Agent Communication | MCP over HTTP | Inter-agent Messaging | ✅ Production |
...
│  └─ /health (Health Check)              │
└─────────────────────────────────────────┘
         │ HTTP/MCP         │ Action Manager
┌──────────────────┐ ┌─────────────────────┐
│ 🔧 Infrastructure│ │  🎯 Action Manager  │
│ Agent (3003)     │ │  ├─ DynamoDB        │
│ ├─ /mcp (HTTP)   │ │  ├─ Queue System    │
│ ├─ /health       │ │  ├─ Workers         │
│ └─ /tools        │ │  └─ WebSocket       │
└──────────────────┘ └─────────────────────┘
         │
...
services:
  ...
  web-app:
    ports: ["3002:3002"]
    environment:
      # WebSocket URL for UI real-time updates (not agent communication)
